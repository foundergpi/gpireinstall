...
create_win_change_password_script() {
    ...
    # Escape password untuk batch file (handle special characters seperti @, !, ^, dll)
    # Di batch file, beberapa karakter perlu di-escape
    # Tapi karena kita pakai quotes di net user command, seharusnya aman
    # Untuk aman, kita gunakan cara yang lebih robust
    
    # Method 1: Direct dengan quotes (simple)
    # Method 2: Download template dari GitHub dan modify (lebih robust untuk special chars)
    
    # Coba download template dulu, jika gagal baru buat manual
    if wget -q "$confhome/windows-change-password-template.bat" -O- >/dev/null 2>&1; then
        info "Downloading password script template from GitHub..."
        wget "$confhome/windows-change-password-template.bat" -O "$target.tmp" 2>/dev/null || {
            warn "Failed to download template, creating script manually..."
            # Fallback ke method manual
            create_password_script_manual "$target" "$password"
            return $?
        }
        
        # Replace PASSWORD_PLACEHOLDER dengan actual password
        # Escape special characters untuk sed (perlu escape &, /, \, dll)
        # Untuk aman, gunakan awk atau perl, tapi sed dengan delimiter alternatif juga bisa
        # Gunakan | sebagai delimiter untuk avoid conflict dengan password characters
        escaped_password=$(printf '%s' "$password" | sed 's/|/\\|/g' | sed 's/&/\\&/g')
        sed "s|PASSWORD_PLACEHOLDER|$escaped_password|g" "$target.tmp" > "$target"
        rm -f "$target.tmp"
        
        # Verify replacement
        if ! grep -q "PASSWORD_PLACEHOLDER" "$target" 2>/dev/null; then
            info "Password placeholder replaced successfully"
        else
            warn "Password placeholder may not be replaced correctly"
        fi
    else
        # Template tidak ada, buat manual
        info "Template not found, creating script manually..."
        create_password_script_manual "$target" "$password"
        return $?
    fi
    ...
    if [ -f "$target" ]; then
        ...
        # Verify script contains password
        if grep -q "$password" "$target" 2>/dev/null || grep -q "Administrator" "$target" 2>/dev/null; then
            info "Password script verified: contains password change command"
        else
            warn "Password script may not contain password change command"
        fi
    else
...
create_password_script_manual() {
    target=$1
    password=$2
    
    # Escape double quotes untuk batch file
    # Batch file menggunakan "" untuk escape double quote
    escaped_password=$(printf '%s' "$password" | sed 's/"/""/g')
    
    cat >$target <<'SCRIPTEOF'
@echo off
...
net user Administrator "PASSWORD_VALUE" >nul 2>&1
if %errorlevel% equ 0 (
    ...
    wmic useraccount where name="Administrator" set password="PASSWORD_VALUE" >nul 2>&1
)
...
SCRIPTEOF
    ...
    # Replace PASSWORD_VALUE dengan actual password
    # Gunakan | sebagai delimiter untuk avoid conflict dengan password characters
    escaped_password_sed=$(printf '%s' "$password" | sed 's/|/\\|/g' | sed 's/&/\\&/g' | sed 's/\//\\\//g')
    sed -i "s|PASSWORD_VALUE|$escaped_password_sed|g" "$target"
    
    # Verify replacement
    if ! grep -q "PASSWORD_VALUE" "$target" 2>/dev/null; then
        info "Password value replaced successfully in manual script"
    else
        warn "Password value may not be replaced correctly in manual script"
    fi
}
